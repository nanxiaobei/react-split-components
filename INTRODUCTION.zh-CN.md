...
仔细想一下：通过闭包，`useMemo` 与 `useRef` 自然就不需要了。

`useMemo` 是因为复杂数据每次都新建，得包一下，而使用闭包，就没有这个必要了。而 `useMemo` 类似 `computed` 的运算机制，改成手动触发即可。也就是将 `useMemo` 的声明式写法变成「手动调用」的命令式写法，这更符合直觉（就像 class 组件时代一样）。

`useRef` 是因为变量每次都新建，得包一下，而使用闭包，是的，变量不存在新建问题，且组件天然持有变量更新后的值，这一切都是 JS 的运行机制而已，一切都自然而然。
